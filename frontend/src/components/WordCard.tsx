...
const WordCard: React.FC<WordCardProps> = ({ term, meaning, phonetic, examples }) => {
      ...
      <div className="card hover:scale-105">
        <div className="card-front flex flex-col rounded-md drop-shadow-md shadow-black" onClick={flipCard}>
          <div className="translate flex w-full h-8 justify-end gap-7 items-center">
            <div className="mt-3 font-bold text-[13px] hover:cursor-pointer hover:underline skip-flip">Translate</div>
          ...
          <div className="audio-word mt-2 flex items-center gap-3 w-full h-12">
            <div className="bg-[#D9D9D9] hover:cursor-pointer hover:scale-110 transition duration-100 flex ml-6 rounded-full w-7 h-7 skip-flip"></div>
            <h3 className=" font-semibold">{term}</h3>
            <h2 className="font-semibold text-[11px]">({phonetic})</h2>
          </div>
          <h1 className="definiton pl-6 font-bold pt-4 text-lg">Definition: </h1>
          ...
          <ul className="examples-list overflow-y-auto max-h-12 md:max-h-40 lg:max-h-40 xl:max-h-40">
            {examples && examples.map((example, index) => (
              <li className="pl-10" key={index}>{example}</li>
            ))}
          </ul>
        ...
        <div className="card-back rounded-md drop-shadow-md shadow-black" onClick={flipCard}>
          <h2>{term}</h2>
